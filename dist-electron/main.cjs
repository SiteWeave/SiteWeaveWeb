...
const require$$5$1 = require("https");
...
function requireMakeDir() {
  ...
  const fs2 = requireFs();
  const { checkPath } = requireUtils$1();
...
const { app, BrowserWindow, Menu, shell, protocol, ipcMain } = require$$1$4;
const path = require$$1$1;
const { createServer } = require$$4$1;
const { parse } = require$$3;
let autoUpdater = null;
try {
  autoUpdater = requireMain().autoUpdater;
} catch (error2) {
  console.warn("electron-updater not available:", error2.message);
  autoUpdater = {
    ...
    if (pathname === "/oauth-data" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          console.error("Error parsing OAuth data:", error2);
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    icon: app.isPackaged ? path.join(process.resourcesPath, "app.asar", "build", "icon.png") : path.join(__dirname, "../build/icon.png"),
    title: "SiteWeave",
    show: false,
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    const indexPath = app.isPackaged ? path.join(process.resourcesPath, "app.asar", "dist", "index.html") : path.join(DIST_PATH || "dist", "index.html");
...
autoUpdater.on("error", (error2) => {
  if (error2.message && (error2.message.includes("latest.yml") || error2.message.includes("404"))) {
    console.log("Update check: No release artifacts found (this is normal for new releases)", error2.message);
    return;
  }
  console.error("Auto-updater error:", error2.message || error2);
});
...
ipcMain.handle("check-for-updates", async () => {
  try {
    ...
    return {
      success: true,
      updateInfo: (result == null ? void 0 : result.updateInfo) ? {
...
ipcMain.handle("exchange-oauth-token", async (event, { provider, code, clientId, redirectUri, codeVerifier }) => {
  const https = require$$5$1;
  const { URL: URL2, URLSearchParams } = require$$3;
  const tokenUrls = {
  ...
  return new Promise((resolve, reject) => {
      ...
      res.on("end", () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          try {
            ...
            reject(new Error(`Failed to parse token response: ${error2.message}`));
          }
        } else {
    ...
    req.on("error", (error2) => {
      reject(new Error(`Token exchange request failed: ${error2.message}`));
    });
