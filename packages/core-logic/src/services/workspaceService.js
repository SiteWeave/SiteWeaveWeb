...
/** Personal workspace with an active 14-day full-feature trial. */
export function isOnPersonalTrial(org) {
  if (!isPersonalWorkspace(org) || !org?.trial_ends_at) return false;
  return new Date(org.trial_ends_at) > new Date();
}

/** Business tier or personal org still within trial window. */
export function hasFullTierAccess(org) {
  return isBusinessWorkspace(org) || isOnPersonalTrial(org);
}

export function getTrialDaysRemaining(org) {
  if (!isOnPersonalTrial(org)) return 0;
...
export function canUseTaskReminders(org) {
  return hasFullTierAccess(org);
}

export function canSendManualPings(org) {
  return hasFullTierAccess(org);
}

export function canUseProgressReports(org) {
  return hasFullTierAccess(org);
}
...
export async function canCreateProject(supabase, organizationId, opts = {}) {
  ...
  const { data: org, error: orgError } = await supabase
    .from('organizations')
    .select('workspace_type, max_projects, lifetime_projects_created, trial_ends_at')
    .eq('id', organizationId)
    .maybeSingle();

  if (orgError || !org) return false;
  if (hasFullTierAccess(org)) return true;
...
export async function canInviteGuestCollaborator(supabase, organizationId, projectId) {
  ...
  const { data: org, error: orgError } = await supabase
    .from('organizations')
    .select('workspace_type, max_guest_collaborators_per_project, trial_ends_at')
    .eq('id', organizationId)
    .maybeSingle();
