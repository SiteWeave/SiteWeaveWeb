...
function emailShell({ subject, branding, bodyHtml }) {
  ...
  const siteweaveFooterBlock = `<table role="presentation" style="width:100%;border-collapse:collapse;">
    <tr>
      <td style="padding:0 10px 0 0;vertical-align:middle;width:34px;">
        <img src="${SITEWEAVE_LOGO_URL}" alt="SiteWeave" width="34" height="34" style="display:block;width:34px;height:34px;border:0;">
      </td>
...
function dailyLogBodyForEmail(log, audienceType) {
  ...
  (sections.work_completed || []).forEach((row) => {
    if (row?.title) lines.push(`• ${row.title}`);
  });
  (sections.weather || []).forEach((row) => {
    if (row?.summary) lines.push(`• Weather: ${row.summary}`);
  });
  if (!isClient) {
    (sections.blockers || []).forEach((row) => {
      if (row?.title) lines.push(`• Issue: ${row.title}`);
    });
  }
  (sections.crew_on_site || []).forEach((row) => {
...
function dailySiteLogsHtml(reportData, schedule, primary) {
  ...
  const audienceType = schedule?.report_audience_type || 'client';
  const showProjectNames = !reportData.project_id && logs.some((l) => l.project_name);
  ...
  return `<div style="margin-bottom:28px;">
    ...
    ${logs.map((log) => {
      ...
      const projectLine = showProjectNames && log.project_name
        ...
        : '';
      return `<div style="margin-bottom:14px;padding:14px 16px;background-color:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;">
        ${projectLine}
...
function pmActionsHtml(reportData, schedule, primary) {
  ...
  if (sections.pm_actions === false) return '';
  const pm = reportData?.pm_actions;
  if (!pm || typeof pm !== 'object') return '';
  const fieldDefs = [
    { key: 'rfi_notes', titleKey: 'pm_actions_rfi' },
    { key: 'long_lead_time_notes', titleKey: 'pm_actions_long_lead' },
    { key: 'change_orders_notes', titleKey: 'pm_actions_change_orders' },
    { key: 'submittals_notes', titleKey: 'pm_actions_submittals' },
  ]
    .map(({ key, titleKey }) => {
      const value = String(pm[key] || '').trim();
      if (!value) return '';
      const title = i18n.t(`progressReports.${titleKey}`);
      return `
  ...
  return `
  <div style="margin-bottom:28px;">
    ${sectionHeading(i18n.t('progressReports.pm_actions_section'), primary)}
    ${fieldDefs}
  </div>`;
}
...
function generateTextVersion(reportData, schedule, period) {
  ...
  const sectionsForText = resolveSections(schedule);
  const pmActions = sectionsForText.pm_actions !== false ? (reportData.pm_actions || null) : null;
  if (pmActions) {
    const parts = [
      { key: 'rfi_notes', titleKey: 'pm_actions_rfi' },
      { key: 'long_lead_time_notes', titleKey: 'pm_actions_long_lead' },
      ...
      .map(({ key, titleKey }) => {
        ...
        return value ? `${i18n.t(`progressReports.${titleKey}`)}:\n${value}` : '';
      })
      .filter(Boolean);
    if (parts.length) text += `${i18n.t('progressReports.pm_actions_section')}\n${parts.join('\n\n')}\n\n`;
  }
